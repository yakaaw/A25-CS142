...
const BAPBList: React.FC = () => {
  ...
  if (loading) return <div className="p-6">Loading BAPB...</div>;

  return (
    <div className="p-6">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-2xl font-semibold">Daftar BAPB</h3>
        <Link to="/bapb/new" className="inline-flex items-center px-3 py-2 bg-indigo-600 text-white rounded hover:bg-indigo-700">
          Buat BAPB baru
        </Link>
      </div>
            ...
            {list.map((b) => (
              <tr key={b.id}>
                ...
                <td className="px-4 py-3 text-sm">
                  <Link to={`/bapb/${b.id}`} className="text-indigo-600 hover:underline">Lihat</Link>
                </td>
              </tr>
            ))}
          </tbody>
